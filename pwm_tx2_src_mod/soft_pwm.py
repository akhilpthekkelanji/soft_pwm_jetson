...
def set_cpu_affinity():
    ...
    try:
        ...
        print("Total number of CPUs = " + str(res))
    except Exception as e:
        print("ERROR during lsmod command")
        print(e)
        return 1
    try:
        pid = os.getpid()
        # do not run image processing on cpu (n-1)
        os.system("sudo taskset -p --cpu-list 0-" + str(n-2 ) +" " + str(pid))
        # get pid of pwm task
        pid = subprocess.check_output(["pidof", "pwm_thread"])
        pid = str(pid)
        pid = pid[2:(len(pid)-3)]
        print("pid of pwm_thread = " + pid)
        # restrict it to cpu (n-1)
        os.system("sudo taskset -p --cpu-list "+ str(n-1) +" " + pid)
    except Exception as e:
...
def init_pwm_port(gpio):
    fname = '/sys/class/soft_pwm/pwm' + gpio + '/pulse'
